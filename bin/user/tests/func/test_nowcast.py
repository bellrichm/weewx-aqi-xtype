...
class TestNowCastCalculate(unittest.TestCase):
    ...
    def test_incomplete_data(self):
        ...
        self.assertEqual(stats.last, None)
        self.assertIsNone(stats.first_time)
        self.assertIsNone(stats.last_time)
        self.assertIsNone(stats.min)
        self.assertIsNone(stats.max)
        self.assertIsNone(stats.min_time)
        self.assertIsNone(stats.max_time)
    ...
    def test_missing_middle_data(self):
        ...
        self.assertEqual(stats.last, aqi_values[-1])
        self.assertEqual(stats.first_time, timestamps[len(aqi_vec)-1])
        self.assertEqual(stats.last_time, timestamps[0])
        self.assertEqual(stats.min, min(aqi_values))
        self.assertEqual(stats.max, max(aqi_values))
        self.assertEqual(stats.min_time, timestamps[aqi_values.index(stats.min)])
        self.assertEqual(stats.max_time, timestamps[aqi_values.index(stats.max)])
        self.assertEqual(start_vec, [timestamps[0]])
        self.assertEqual(stop_vec, [timestamps[0] + 3600])
        self.assertEqual(aqi_vec, aqi_values)
